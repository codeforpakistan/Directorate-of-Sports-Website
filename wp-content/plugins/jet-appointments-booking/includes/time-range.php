...
class Time_Range {
	...
	public static function get_settings( $args = [] ){
		$settings = [
			...
			'minute_increment'   => apply_filters( 'jet-appointments/time-picker/config/minute-increment', 5 ),
		];
	...
	public static function get_working_hours_list( $args, $returnType = 'string' ){
		$weekday       = strtolower( date( 'l', $args['date'] ) );
		$working_hours = Tools::get_time_settings( $args['service'], $args['provider'], 'working_hours' );
		
		if( $returnType !== 'string'){
			return empty( $working_hours[ $weekday ] ) ? [] : $working_hours[ $weekday ] ;
		}

		return self::hours_array_to_list( $working_hours[ $weekday ], 'from', 'to' );
	}
	...
	public static function get_busy_hours_list( $args, $returnType = 'string' ){
		$query = [
			...
			'status'   => array_merge( Plugin::instance()->statuses->valid_statuses() ),
		];
		$order = [
			'orderby' => 'slot',
			'order'   => 'ASC',
