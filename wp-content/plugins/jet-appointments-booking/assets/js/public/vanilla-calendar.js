	...
	function VanillaCalendar( options ) {
		...
		const isAvailableDay = function( args = {} ) {
			let {
				...
				isAvailable = true,
				weekDay = getWeekDay ( new Date( date * 1000 ).getDay() );
		...
		const addMonth = function ( date, months ) {
			let output = new Date( date * 1000 );
			output.setUTCMonth( output.getMonth() + months );
		...
		const setYear = function ( date, years ) {
			let output = new Date( date * 1000 );
			output.setUTCFullYear( output.getFullYear() + years );
			return output / 1000;
		}
		...
		this.init = function () {
			...
			let form                 = instance.closest( 'form' );
